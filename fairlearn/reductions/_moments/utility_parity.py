...
class UtilityParity(ClassificationMoment):
    ...
    def load_data(
        ...
        *,
        ...
        self.tags = self.tags.with_columns(
            nw.from_native(event, series_only=True).alias(_EVENT)
        )
        ...
        self.prob_event = (
            self.tags.group_by(_EVENT).agg(nw.len())["len"] / self.total_samples
        )

        self.prob_group_event = (
            self.tags.group_by([_EVENT, _GROUP_ID]).agg(nw.len())["len"]
            / self.total_samples
        )
